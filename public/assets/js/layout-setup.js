...
const settings = [
    { attribute: "data-layout", defaultValue: "horizontal", valid: ["horizontal"] },
    { attribute: "data-bs-theme", defaultValue: "light", valid: ["light", "dark"] },
    { attribute: "data-content-width", defaultValue: "default" },
    { attribute: "dir", defaultValue: "ltr", valid: ["ltr", "rtl"] },
    { attribute: "data-sidebar-size", defaultValue: "default" },
    { attribute: "data-sidebar-color", defaultValue: "default" },
    { attribute: "data-sidebar", defaultValue: "light", valid: ["light", "dark"] },
...
settings.forEach((setting) => {
    let savedValue = localStorage.getItem(setting.attribute);

    // If saved value is invalid or missing → use default
    if (!savedValue || (setting.valid && !setting.valid.includes(savedValue))) {
        savedValue = setting.defaultValue;
    }

    // Apply to HTML element
    htmlElement.setAttribute(setting.attribute, savedValue);

    if (setting.attribute === "dir") {
        updateLayoutDir(savedValue);
    } else if (setting.attribute === "data-bs-theme") {
        setTheme(savedValue);
    }

    // Match radio button
    const radioSelector = `input[name="${setting.attribute}"][value="${savedValue}"]`;
...
function updateSimpleBar(value) {
    ...
    setTimeout(() => {
        ...
        if ((value === "horizontal" || value === "semibox") && document.documentElement.getAttribute("data-sidebar") !== "icon") {
