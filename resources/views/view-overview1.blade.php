        ...
        <div class="modal-dialog">
            <div class="modal-content">
                <form id="editProductForm">
                    @csrf
                    <input type="hidden" name="edit_id" id="edit_product_id">
                    ...
                    <div class="modal-body">
                        ...
                        <div class="mb-3">
                            ...
                            <select name="station_id" id="edit_station_id" class="form-control" disabled></select>
                        </div>

                        <!-- Product (readonly display) -->
                        <div class="mb-3">
                            <label class="form-label">Product</label>
                            <select name="product_id" id="edit_product_id_select" class="form-control" disabled></select>
                        </div>

                        <!-- Select Shift (NEW) -->
                        <div class="mb-3">
                            <label class="form-label">Select Shift</label>
                            <select name="shift_id" id="edit_shift_select" class="form-control">
    ...
    <script>
        ...
        function editProduct(id) {
            $.get(`/api/station-product/${id}`, function (res) {
                ...
                // ✅ Hidden ID
                $('#edit_product_id').val(res.station_product_id);

                // ✅ Station dropdown (disabled, sirf display)
                let stationSelect = document.querySelector('#edit_station_id');
                if (stationSelect) {
                    if (stationSelect.choicesInstance) stationSelect.choicesInstance.destroy();
                    stationSelect.innerHTML = `<option value="${res.station_id}" selected>${res.station_name}</option>`;
                    stationSelect.choicesInstance = new Choices(stationSelect, { searchEnabled: false, shouldSort: false });
                    stationSelect.choicesInstance.setChoiceByValue(String(res.station_id));
                }

                // ✅ Product dropdown (disabled, sirf display)
                let productSelect = document.querySelector('#edit_product_id_select');
                if (productSelect) {
                    if (productSelect.choicesInstance) productSelect.choicesInstance.destroy();
                    productSelect.innerHTML = `<option value="${res.product_id}" selected>${res.product_name}</option>`;
                    productSelect.choicesInstance = new Choices(productSelect, { searchEnabled: false, shouldSort: false });
                    productSelect.choicesInstance.setChoiceByValue(String(res.product_id));
                }

                // ✅ Price
                $('#editProductModal input[name="price"]').val(res.price);

                // ✅ Effective From
                if (res.effective_from) {
                    let formattedFrom = res.effective_from.replace(' ', 'T').slice(0, 16);
                    $('#editProductModal input[name="effective_from"]').val(formattedFrom);
                }

                // ✅ Effective To
                if (res.effective_to) {
                    let formattedTo = res.effective_to.replace(' ', 'T').slice(0, 16);
                    $('#editProductModal input[name="effective_to"]').val(formattedTo);
                }

                // ✅ Load Open Shifts for this station
                let stationId = `{{ request()->segment(2) }}`;

                $.get(`/api/stations/${stationId}/open-shifts`, function (shiftRes) {
                    let shiftSelect = document.querySelector('#edit_shift_select');

                    // Destroy previous Choices instance
                    if (shiftSelect.choicesInstance) {
                        shiftSelect.choicesInstance.destroy();
                        shiftSelect.choicesInstance = null;
                    }

                    let shifts = (shiftRes && shiftRes.data) ? shiftRes.data : [];

                    // ✅ Sirf actual shifts ka array banao (placeholder manually nahi)
                    let choicesArray = shifts.map(s => ({
                        value: String(s.id),
                        label: `Shift #${s.shift_no || s.id} (${s.start_time || ''} - ${s.end_time || 'open'})`,
                        selected: res.shift_id && String(res.shift_id) === String(s.id)
                    }));

                    shiftSelect.choicesInstance = new Choices(shiftSelect, {
                        searchEnabled: false,
                        shouldSort: false,
                        placeholder: true,
                        placeholderValue: '-- Select Shift --',   // ✅ yahi placeholder use karega
                        allowHTML: false
                    });

                    // ✅ setChoices mein sirf real shifts daalo
                    shiftSelect.choicesInstance.setChoices(choicesArray, 'value', 'label', true);

                    // Agar res.shift_id exist karta hai toh select karo
                    if (res.shift_id) {
                        shiftSelect.choicesInstance.setChoiceByValue(String(res.shift_id));
                    }
                }).fail(function () {
                    showToast('Error loading open shifts', 'error');
                });

                // ✅ Show modal
                $('#editProductModal').data('edit-id', res.station_product_id).modal('show');

            }).fail(() => showToast('Error fetching product details', 'error'));
        }
        ...
        $('#editProductForm').off('submit').on('submit', function (e) {
            ...
            let stationEl = document.querySelector('#edit_station_id');
            let productEl = document.querySelector('#edit_product_id_select');
            let shiftEl = document.querySelector('#edit_shift_select');

            let station_id = stationEl?.choicesInstance ? stationEl.choicesInstance.getValue(true) : $(stationEl).val();
            let product_id = productEl?.choicesInstance ? productEl.choicesInstance.getValue(true) : $(productEl).val();
            let shift_id = shiftEl?.choicesInstance ? shiftEl.choicesInstance.getValue(true) : $(shiftEl).val();

            if (!station_id || !product_id) {
                showToast('Station or Product missing', 'error');
                return;
            }
            ...
            let data = {
                ...
                price: $('#editProductModal input[name="price"]').val(),
                effective_from: $('#editProductModal input[name="effective_from"]').val(),
                effective_to: $('#editProductModal input[name="effective_to"]').val()
            };
